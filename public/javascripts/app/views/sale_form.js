define(['jquery', 
  ...
  $(document).ready(function() {
    SaleFormView = Backbone.View.extend({
      initialize: function(options) {
        ...
        $('.submit-form').click(function() {
          var errors = [];
          errors.push({'title': 'Bill Customer Errors', 'errors': saleForm.options['billCustomer'].validateForm()});
          errors.push({'title': 'Purchase Customer Errors', 'errors': saleForm.options['purchaserCustomer'].validateForm()});
          errors.push({'title' : 'Property Info Errors', 'errors': saleForm.options['propertyInfo'].validateForm()});
      ...
      submitForm: function() {
        ...
        $.extend(saleParams, {'billCustomer': saleForm.options['billCustomer'].getCustomerValues()});
        $.extend(saleParams, {'purchaserCustomer': saleForm.options['purchaserCustomer'].getCustomerValues()});
        $.extend(saleParams, {'propertyInfo': saleForm.options['propertyInfo'].getPropertyValues()});
        $.extend(saleParams, {'marketingValues': saleForm.getMarketingValues()});
        $.extend(saleParams, {'paymentSchedule': saleForm.getSchedule()});
 
        // Get Co-Broker name
        if ($('#activate-create-co-broker:checked').val() != undefined) {
          $.extend(saleParams, {'coBroker': $('#new-co-broker').val()});
        } else {
          $.extend(saleParams, {'coBroker': $('#co_broker').val()});
        }
    ...
    var purchaserCustomerView = new CustomerView({formType: 'purchaser'});
    var propertyView = new PropertyView();
    saleForm = new SaleFormView({'el': '#deals-form-container', 'billCustomer': billCustomerView, 'purchaserCustomer': purchaserCustomerView, 'propertyInfo': propertyView});
  });
});
